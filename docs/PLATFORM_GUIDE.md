...
Implémenté via **`@adkit.so/meta-pixel-next`** (PageView automatique sur changement de route, TypeScript natif).

**Composant :** `src/components/vendor-shop/providers/MetaPixelProvider.tsx`
Architecture : `<MetaPixel pixelId>` (librairie) → `<InnerPixelProvider>` (vérifie `enabledEvents`, log Convex).

| Événement | Méthode | Déclencheur |
|-----------|---------|-------------|
| `PageView` | JavaScript — auto `@adkit.so` | Chaque changement de route `/shop/[slug]` |
| `ViewContent` | JavaScript — `meta.track()` | Ouverture fiche produit |
| `AddToCart` | JavaScript — `meta.track()` | Ajout panier (`ShopProductCard`) |
| `InitiateCheckout` | JavaScript — `meta.track()` | Ouverture `QuickOrderSheet` |
| `Purchase` | **Serveur (CAPI)** | Webhook Moneroo → `meta/mutations.trackPurchase` |

**Sélection d'événements :** Le vendeur peut activer/désactiver chaque événement depuis `/vendor/store/meta`. Les choix sont stockés dans `stores.meta_pixel_enabled_events` (tableau de strings). `null` = tous activés. `MetaPixelProvider` vérifie ce tableau avant tout appel `meta.track()` et avant tout log Convex.

**Dashboard analytics intégré :** La page `/vendor/store/meta` affiche un funnel en barres (data de `meta_pixel_events`) avec sélecteur de période. Chaque step indique son count et son taux de conversion par rapport à l'étape précédente.

**Scoping pixel :** Les events sont loggés avec le `pixel_id` actif au moment de l'event. Si le vendeur change de Pixel ID, les anciennes données ne sont plus visibles dans le funnel (elles restent en base mais sont filtrées par `pixel_id`).
